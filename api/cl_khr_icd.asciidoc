...
point. In order to distinguish ICD 1 and ICD 2 objects, the pointer to
`clGetPlatformIDs` in the dispatch structure must be set to
`CL_ICD2_TAG_KHR`. The vendor driver is responsible for propagating the
`dispatch_data` value when returning objects to users.
...
Used as a value in the pointer to `clGetPlatformIDs` in
